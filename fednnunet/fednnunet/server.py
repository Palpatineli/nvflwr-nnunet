...
StateDict = dict[str, torch.Tensor | list[int] | dict[str, int | float | bool | None]]
LayerDict = dict[str, torch.Tensor]
...
def average_dicts(dicts: list[dict[str, int | float]] | None) -> dict[str, float]:
    if not dicts:
        return {}
    totals: dict[str, float] = defaultdict(lambda: 0)
...
@overload
def weighted_mean(values: Sequence[torch.Tensor], weights: Sequence[float | int]) -> torch.Tensor:...

@overload
def weighted_mean(values: Sequence[float | int], weights: Sequence[float | int]) -> float:...

def weighted_mean(values: Sequence[torch.Tensor | float | int], weights: Sequence[float | int]) -> torch.Tensor | float:
    return sum(value * weight for value, weight in zip(values, weights)) / sum(weights)


def max(values, weights):  # pyright: ignore[reportUnusedParameter]
    values = torch.tensor(values)
    return torch.max(values).item()


def min(values, weights):  # pyright: ignore[reportUnusedParameter]
    values = torch.tensor(values)
    return torch.min(values).item()

T = TypeVar('T')

def concatenate(values: list[Sequence[T]], weights) -> list[T]:  # pyright: ignore[reportUnusedParameter]
...
def aggregate_fingerprints(parameters: list[Parameters]) -> Parameters:
    ...
    for key in state_dicts[0].keys():
        if isinstance(subdict := state_dicts[0][key], dict):
            new_state_dict[key] = {}
            for subkey in subdict.keys():
                if isinstance(subdict2 := subdict[subkey], dict):
                    new_state_dict[key][subkey] = {}
                    for subsubkey in subdict2.keys():  # pyright: ignore[reportUnknownVariableType]
                        new_state_dict[key][subkey][subsubkey] = aggregation_dict[
                            subsubkey
                        ](
                            [sd[key][subkey][subsubkey] for sd in state_dicts],  # pyright: ignore[reportOptionalSubscript,reportCallIssue,reportArgumentType,reportIndexIssue]
                            num_samples,
                        )
                else:
                    new_state_dict[key][subkey] = aggregation_dict[subkey](
                        [sd[key][subkey] for sd in state_dicts], num_samples  # pyright: ignore[reportCallIssue,reportArgumentType]
                    )
        else:
            new_state_dict[key] = aggregation_dict[key](
                    [cast(torch.Tensor, sd[key]) for sd in state_dicts], num_samples  # pyright: ignore[reportArgumentType]
            )
...
class MyStrategy(fl.server.strategy.FedAvg):
    def __init__(
        ...
        *,
        ...
        evaluate_fn: EvaluationFn | None = None,
    ...
    def find_common_layers(self, state_dicts: list[LayerDict]) -> list[str]:
    ...
    def create_compatible_state_dict(self, state_dicts: list[LayerDict],
                                     compatible_keys: list[str]) -> LayerDict:
        new_state_dict: LayerDict = {}
        for key in compatible_keys: # Assuming we take the parameters from the first state_dict
    ...
    def configure_fit(
        ...
        parms = [
            parameters_to_state_dict(
                client.get_parameters(
                    ins=GetParametersIns(config=fit_ins.config), timeout=None, group_id=None
                ).parameters
            )
        ...
        compatible_keys = self.find_common_layers(cast(list[LayerDict], parms))

        # here we are doing the merging already...
        new_state_dict = self.create_compatible_state_dict(cast(list[LayerDict], parms), compatible_keys)

        # Return client/config pairs
        return [(client, fit_ins) for client in clients]

    Results = list[tuple[ClientProxy, fl.common.FitRes] | None]
    ...
    def aggregate_fit(  # pyright: ignore[reportIncompatibleMethodOverride]
        ...
        server_round: int,
        results: GoodResults,
        failures: MaybeResults,
    ) -> tuple[list[float], dict[str, Scalar]] | tuple[Parameters, dict[str, Scalar]] | None:
        if failures:
            fl.common.logger.log(2, f"Round {server_round} had {len(failures)} failures.")
        ...
        averages = average_dicts([cast(dict[str, float], r[1].metrics) for r in successful_results])
        return aggregated_weights, averages  # pyright: ignore[reportReturnType]

    def aggregate_weights(self, results: GoodResults) -> Parameters:
        dicts = cast(list[dict[str, torch.Tensor]], [parameters_to_state_dict(res[1].parameters) for res in results])
